...
def validate_user():
    ...
    if not correct_length(username):
        username_error = "User name must be between 4 and 20 characters long, and cannot have spaces."
        username=''
        return redirect('signup_form.html',username='',username_error=username_error,user_password='',
        user_password_error='',verify_user_password='',
    ...
    if not correct_length(user_password):
        ...
        user_password=''
        return redirect('signup_form.html',username=username,username_error='',user_password='',
        user_password_error=user_password_error,verify_user_password='',
        user_email='',user_email_error='')
    if not passwords_match(user_password, verify_user_password):
        ...
        return redirect('signup_form.html',username=username,username_error=username_error,user_password='',
        user_password_error=user_password_error,verify_user_password='',
        user_email='',user_email_error='')
    ...
    if not user_email == '':
        if not proper_email(user_email):
            ...
            return redirect('signup_form.html',username=username,username_error=username_error,user_password='',
            user_password_error=user_password_error,verify_user_password='',
            user_email=user_email,user_email_error=user_email_error)      
        else: 
